...
function LoadConfig() {
	...
	Scripts.BattleOrders = true;
		...
		Config.BattleOrders.Getters = ["Choppadex","Choppadef","Choppastiltskin"]; // List of players to wait for before casting Battle Orders (mode 0). All players must be in the same area as the BOer.
	...
	*/
	...
	Config.Leader = "Choppadex"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	Config.QuitList = ["Choppadex","Pageof_Sunder","Choppadef","Choppastiltskin"]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"];
	Config.QuitListMode = 0; // 0 = use character names; 1 = use profile names (all profiles must run on the same computer).
	Config.QuitListDelay = [2,4]; // Quit the game with random delay in case of using Config.QuitList. Example: Config.QuitListDelay = [1, 10]; will exit with random delay between 1 and 10 seconds.
	...
	Scripts.DiabloHelper = true; // Chaos helper, kills monsters and doesn't open seals on its own.
		Config.DiabloHelper.Wait = 320; // Seconds to wait for a runner to be in Chaos. If Config.Leader is set, it will wait only for the leader.
	...
	Scripts.BaalHelper = true;
		Config.BaalHelper.Wait = 320; // Seconds to wait for a runner to be in Throne
	...
	Scripts.BaalAssistant = false; // Used to leech or help in baal runs.
		Config.BaalAssistant.Wait = 320; // Seconds to wait for a runner to be in the throne / portal wait / safe TP wait / hot TP wait...
	...
	Config.MinGameTime = 160; // Min game time in seconds. Bot will TP to town and stay in game if the run is completed before.
	Config.MaxGameTime = 2990; // Maximum game time in seconds. Quit game when limit is reached.
	...
	Config.ScanShrines = [20, 18, 1, 2, 3, 15, 12, 6, 10, 8, 9, 11, 13];
